...
const HomePage = () => {
    ...
    //* values
    let [personnelData, setPersonnelData] = useState({ owner: '', assistant: '', customer: '' }),
    ...
    useEffect(() => { console.log(isHavePro) }, [isHavePro, isLoading])
    useEffect(() => { handleEvent.calTotal() }, [createServiceItem, total])
    return (
        <>
            <Loading isLoading={isLoading} />
            <div id='printPage'>
                <div className='container' style={thisStep == 'one' ? { width: '500px' } : { width: '300px', fontSize: '24px' }}>
                    <div className='row justify-content-center p-3'>
                        {/* <img src={require('../assets/image/logo.png')} style={{ width: '200px' }} /> */}
                        <h1 className='text-center'>Na Mi 沙龍</h1>
                        <p className='text-end mt-3' style={{ fontSize: '18px' }}>{moment().format('YYYY/MM/DD HH:mm:ss')}</p>
                        {
                            thisStep == 'one' &&
                            <>
                                <div className='mb-3'>
                                    ...
                                    <div className='row g-0 align-items-center mt-2'>
                                        <div className='col-2'>
                                            <label for='designer' className='col-form-label'>助理</label>
                                        </div>
                                        <div className='col-auto'>
                                            <select className="form-select" aria-label="Default select example"
                                                onChange={e => setPersonnelData({ ...personnelData, assistant: e.target.value })}>
                                                <option selected disabled>請選擇助理</option>
                                                {
                                                    ...
                                                    <select className="form-select" aria-label="Default select example" onChange={e => handleEvent.chooseServiceItem(item.id, e)}>
                                                        <option selected disabled>請選擇服務項目</option>
                                                        {
                                                            serviceItem.map((item, index) => {
                                                                return (
                                                                    <option value={`${item.itemName}-${item.fee}-${item.isHaveFee}`}>{item.itemName}</option>
                                                                )
                                                            })
                                                        }
                                                    </select>
                                                ...
                                                <div className='col-3 text-end'>
                                                    {
                                                        item.isHaveFee == 'false' ?
                                                            <input type='text' className='form-control text-end fw-bolder' onChange={e => handleEvent.setFee(item.id, e)} />
                                                            :
                                ...
                                <div>
                                    <div className='d-flex'>
                                        ...
                                        <div class="form-check mx-2">
                                            <input class="form-check-input" type="radio" name="flexRadioDefault" id="flexRadioDefault1" onClick={e => setIsHavePro(true)} />
                                            <label class="form-check-label" for="flexRadioDefault1">
                                                是
                                            </label>
                                        </div>
                                        <div class="form-check mx-2">
                                            <input class="form-check-input" type="radio" name="flexRadioDefault" id="flexRadioDefault2" checked onClick={e => setIsHavePro(false)} />
                                ...
                                <div className='text-center mb-2'>
                                    <button className='btn btn-primary' onClick={handleEvent.sendEvent}>送出</button>
                                </div>
                            </>
                        }
                        {
                        ...
                        <p className='small text-center mt-2' style={{ fontSize: '14px' }}>Copyright © 2022 NaMi Hair Salon. All rights reserved.</p>
                    </div>
                </div>
            </div>
            {
                ...
                <div className='text-center'>
                    <button className='btn btn-primary mx-1' onClick={e => window.location.reload()}>返回</button>
                    <button className='btn btn-primary mx-1' onClick={e => handleEvent.printEvent()}>列印</button>
                </div>
            }
        </>
    )
